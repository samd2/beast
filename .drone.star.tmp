...
addon_base = { "apt": { "packages": [ "software-properties-common", "libffi-dev", "libstdc++6", "binutils-gold", "gdb", "mlocate" ] } }
...
def main(ctx):
  ...
  linux_cxx("Clang 11", "clang++-11", packages="clang-11 libstdc++-9-dev", llvm_os="bionic", llvm_ver="11", buildtype="boost", buildscript="drone", image="cppalliance/droneubuntu2004:1", environment={'VARIANT': 'release', 'TOOLSET': 'clang', 'COMPILER': 'clang++-11', 'CXXSTD': '17,2a'}, globalenv=globalenv),
  ...
  linux_cxx("GCC 9", "g++-9", packages=" ".join(addon_base["apt"]["packages"]) + " g++-9", buildtype="boost", buildscript="drone", image="cppalliance/droneubuntu2004:1", environment={'VARIANT': 'release', 'TOOLSET': 'gcc', 'COMPILER': 'g++-9', 'CXXSTD': '17,2a'}, globalenv=globalenv),
  linux_cxx("GCC 10", "g++-10", packages=" ".join(addon_base["apt"]["packages"]) + " g++-10", buildtype="boost", buildscript="drone", image="cppalliance/droneubuntu2004:1", environment={'VARIANT': 'release', 'TOOLSET': 'gcc', 'COMPILER': 'g++-10', 'CXXSTD': '17,2a'}, globalenv=globalenv),
