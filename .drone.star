...
def main(ctx):
  return [
  linux_cxx("docs", "g++", packages="docbook docbook-xml docbook-xsl xsltproc libsaxonhe-java default-jre-headless", buildtype="docs", buildscript="drone", image="cppalliance/droneubuntu1604:1", environment={'DRONE_JOB_UUID': 'b6589fc6ab'}, globalenv=globalenv),
  ...
  linux_cxx("Default clang++ with libc++", "clang++-libc++", packages=" ".join(addon_base["apt"]["packages"]), buildtype="boost", buildscript="drone", image=linuxglobalimage, environment={'VARIANT': 'debug', 'TOOLSET': 'clang', 'COMPILER': 'clang++-libc++', 'CXXSTD': '11', 'CXX_FLAGS': '<cxxflags>-stdlib=libc++ <linkflags>-stdlib=libc++', 'DRONE_JOB_UUID': 'da4b9237ba'}, globalenv=globalenv),
  ...
  osx_cxx("VARIANT=debug TOOLSET=clang COMPILER=clang++ Job 7", "clang++", packages="", buildtype="boost", buildscript="drone", environment={'VARIANT': 'debug', 'TOOLSET': 'clang', 'COMPILER': 'clang++', 'CXXSTD': '14', 'DRONE_JOB_UUID': '902ba3cda1'}, globalenv=globalenv),
    ]
